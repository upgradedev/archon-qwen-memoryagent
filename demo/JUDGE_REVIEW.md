...
| Working Alibaba deployment | [`deploy/DEPLOY_STATE.md`](../deploy/DEPLOY_STATE.md), project-contained controller evidence, public `/health` + `/ready` | **Ready for current runtime source:** `0910ab7…` is live-verified from attempt 27 under the reviewed `terminal-success-truncated-output` mode; final media capture remains pending and must bind to that evidence |
