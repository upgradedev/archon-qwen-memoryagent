...
This is how the entry runs on Alibaba Cloud: a single ECS instance (`ecs.e-c1m2.large`, ap-southeast-1) runs docker-compose with the backend container plus a self-hosted `pgvector/pgvector` container as the memory store, behind one public URL. Exact runtime source [`cfd485de1dd01473c8d6be91521e5560d8e8313e`](https://github.com/upgradedev/archon-qwen-memoryagent/commit/cfd485de1dd01473c8d6be91521e5560d8e8313e) is live-verified from project-contained exact-deploy attempt 26 on 2026-07-18. Alibaba Cloud Assistant is terminal `Success` with exit code `0`, and the retained output ends at the SHA-bound application marker, so the reviewed evidence mode is `terminal-success-truncated-output`, not strict aggregate-final-marker evidence. The controller verified immutable checkout, image build, schema/grants, cross-application denial, real-Qwen readiness, grounded recall and zero-residue cleanup. [`deploy/DEPLOY_STATE.md`](./deploy/DEPLOY_STATE.md) is the authoritative release record; final gallery/video capture must bind to that SHA and its retained controller evidence.
