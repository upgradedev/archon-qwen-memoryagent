...
The claim is not "we detect and they cannot." It is **"the audit recommends without mutating, explainably and portably"**, followed—only when requested—by an explicit atomic human decision.
...
sequence use—never ownership, DDL, `TRUNCATE`, temporary tables, role creation,
