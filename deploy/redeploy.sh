...
#                recording). DESTRUCTIVE — wipes stored memories. PATH-B only.
...
HAS_DB_SVC=0
if compose ps --services 2>/dev/null | grep -qx "$DB_SVC"; then HAS_DB_SVC=1; fi
...
if [ "$HAS_DB_SVC" -eq 1 ]; then
  ...
  for i in $(seq 1 30); do
    if compose exec -T "$DB_SVC" pg_isready -U postgres >/dev/null 2>&1; then
...
if ! compose run --rm "$BACKEND_SVC" npm run db:schema; then
...
if [ "$TRUNCATE" -eq 1 ]; then
  ...
  compose exec -T "$DB_SVC" psql -U postgres -d postgres -c 'TRUNCATE agent_memory;' \
    || die "TRUNCATE failed."
...
JSON
  ...
  if [ "$HAS_DB_SVC" -eq 1 ]; then
    compose exec -T "$DB_SVC" psql -U postgres -d postgres \
      -c "DELETE FROM agent_memory WHERE company = '$SMOKE_COMPANY';" >/dev/null 2>&1 \
