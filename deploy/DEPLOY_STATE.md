...
> evidence-scoped recall with `company=Northwind Trading` and `limit=3`, plus the
> bounded semantic audit with `company=Northwind Trading`, `kind=insight`, and
> `maxPairs=1`; final gallery/video capture must bind to this exact SHA and
> deployment evidence.
>
...
> passed exact deploy attempt 22, and the still-earlier
